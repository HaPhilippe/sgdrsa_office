...
export default function Departement_edit_page() {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (isValidate()) {
        ...
        const res = await fetchApi(`/rh/departement/update/${ID_DEPARTEMENT}`, {
          method: "PUT",
          body: form,
        });
        dispacth(
          setToastAction({
            severity: "success",
            summary: "Departement modifié",
            detail: "Le departement a été modifié avec succès",
            life: 3000,
          })
        );
  ...
  const fetchEmployeur = useCallback(async () => {
    try {
      const res = await fetchApi("/rh/employeur/fetchemployeur")
      setEmployeur(res.result.data.map(emplo => {
        return {
          name: emplo.ENTREPRISE,
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchApi(`/rh/departement/find/${ID_DEPARTEMENT}`);
  ...
  useEffect(() => {
    dispacth(setBreadCrumbItemsAction([
      ...
      administration_routes_items.edit_departement
    ]))
    return () => {
      dispacth(setBreadCrumbItemsAction([]))
    }
  ...
  return (
    <>
      ...
      <div className="px-4 py-3 main_content bg-white has_footer">
        <div className="">
          <h1 className="mb-3">Modifier departement</h1>
          <hr className="w-100" />
        </div>
