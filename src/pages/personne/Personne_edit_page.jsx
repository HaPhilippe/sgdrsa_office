...
export default function Personne_edit_page() {
  const dispacth = useDispatch();
  const [data, handleChange, setData, setValue] = useForm(initialForm);
  const [showCalendar, setShowCalendar] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const navigate = useNavigate();
  const { ID_PERS } = useParams();
  const [personne, setPersonne] = useState(null);
  const [loadingPersonne, setLoadingPersonne] = useState(true);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (isValidate()) {
        ...
        const res = await fetchApi(`/rh/personne/update/${ID_PERS}`, {
          method: "PUT",
          body: form,
        });
        dispacth(
          setToastAction({
            severity: "success",
            summary: "Personne modifié",
            detail: "La personne a été modifié avec succès",
            life: 3000,
          })
        );
        navigate("/personne");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchApi(`/rh/personne/find/${ID_PERS}`);
       
        const pers = res.result;
        setPersonne(pers);
        setData({
          NOM: pers.NOM,
          PRENOM: pers.PRENOM,
  ...
  useEffect(() => {
    dispacth(
      setBreadCrumbItemsAction([
        ...
        administration_routes_items.edit_personne
      ])
    );
    return () => {
